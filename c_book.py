...
class Book:
    ...
    def insert(self):
        insertData([{"titolo" : str(self.title), "autore" : str(self.author), "prezzo" : str(self.price), "ISBN" : str(self.isbn), "copie_magazzino" : str(self.copies), "riserva_noleggio" : str(self.reserved), "copie_noleggiate" : str(self.rented)}], "libro")

    def delete(self):
        recDelete("libro", {"ISBN" : str(self.isbn)})

    def is_rentable(self):
        result=recSelect("libro", {"ISBN" : str(self.isbn)}, "copie_magazzino")
        if result:
            return True
        else:
            return False
    ...
    def is_low(self):
        copie_magazzino=recSelect("libro", {"ISBN" : str(self.isbn)}, "copie_magazzino")[0].get("copie_magazzino")
        riserva_noleggio=recSelect("libro", {"ISBN" : str(self.isbn)}, "riserva_noleggio")[0].get("riserva_noleggio")
        copie_noleggiate=recSelect("libro", {"ISBN" : str(self.isbn)}, "copie_noleggiate")[0].get("copie_noleggiate")
